...
import PrimaryContact from '../models/PrimaryContact.model';
...
const recalcOutstanding = async (bookingId: string) => {
    ...
    if (booking) {
        // totalAmount is the single source of truth for selling price
...
export const getRecentBookings = asyncHandler(async (req: Request, res: Response) => {
    ...
    const mapped = bookings.map(b => {
        const seg0 = (b as any).segments?.[0];
        return { 
            ...b, 
            ...
            destinationCity: seg0?.country || seg0?.to || null,
            destination: seg0?.country || seg0?.to || null,
            travelDate: seg0?.departureDate || null,
            amount: (b as any).totalAmount || 0,
...
export const getBookingById = asyncHandler(async (req: Request, res: Response) => {
    ...
    const fetchPromise = (async () => {
        ...
        // Derive legacy flat fields from segments for backward compat
        ...
        return {
            ...
            destination: seg0?.country || seg0?.to || null,
            travelDate: seg0?.departureDate || null,
            returnDate: seg0?.returnDate || null,
            flightFrom: seg0?.from || null,
            flightTo: seg0?.to || null,
            tripType: seg0?.tripType || 'one-way',
            amount: (booking as any).totalAmount || 0,
...
export const createBooking = asyncHandler(async (req: Request, res: Response) => {
    ...
    // Build segments from input (prioritize segments[], fallback to legacy flat fields)
    let segments = (result.data.segments || []).map((s: any) => ({
        from: s.from || '',
        to: s.to || '',
        ...
        tripType: s.tripType || result.data.tripType || 'one-way',
        country: s.country || finalDestination || null,
    }));

    // Fallback: if no segments provided but legacy flat fields exist, build segment[0]
    if (segments.length === 0 && (result.data.flightFrom || result.data.flightTo || finalTravelDate)) {
        segments = [{
            from: result.data.flightFrom || '',
            to: result.data.flightTo || '',
            departureDate: finalTravelDate,
            returnDate: null,
            returnDepartureTime: null,
            tripType: result.data.tripType || 'one-way',
            country: finalDestination || null,
        }];
    }
    ...
    const booking = await Booking.create({
        ...
        segments,
        totalAmount: result.data.totalAmount || result.data.amount || 0,
...
export const updateBooking = asyncHandler(async (req: Request, res: Response) => {
    ...
    if (req.body.segments) {
        ...
        // Remove legacy flat field writes since segments is the source of truth
        delete updateData.flightFrom;
        delete updateData.flightTo;
        delete updateData.travelDate;
        delete updateData.returnDate;
        delete updateData.tripType;
        delete updateData.destination;
    } else if (req.body.flightFrom !== undefined || req.body.flightTo !== undefined || req.body.travelDate !== undefined) {
        // Legacy flat field update — convert to segments[0] update
        // We need to fetch current segments first to avoid overwriting
        const currentBooking = await Booking.findById(id).select('segments').lean();
        const currentSeg = (currentBooking as any)?.segments?.[0] || {};
        updateData.segments = [{
            from: req.body.flightFrom ?? currentSeg.from ?? '',
            to: req.body.flightTo ?? currentSeg.to ?? '',
            departureDate: req.body.travelDate ? new Date(req.body.travelDate) : (currentSeg.departureDate || null),
            returnDate: req.body.returnDate ? new Date(req.body.returnDate) : (currentSeg.returnDate || null),
            returnDepartureTime: req.body.returnDepartureTime ?? currentSeg.returnDepartureTime ?? null,
            tripType: req.body.tripType ?? currentSeg.tripType ?? 'one-way',
            country: req.body.destination ?? currentSeg.country ?? null,
        }];
        // Clean up flat field keys from updateData
        delete updateData.flightFrom;
        delete updateData.flightTo;
        delete updateData.travelDate;
        delete updateData.returnDate;
    ...
    // Normalize amount → totalAmount
    if (req.body.amount !== undefined && req.body.totalAmount === undefined) {
        updateData.totalAmount = req.body.amount;
        delete updateData.amount;
    }
    // Clean up other legacy keys that shouldn't be persisted
    delete updateData.includesFlight;
    delete updateData.includesAdditionalServices;
    delete updateData.pricePerTicket;
    delete updateData.travellers;
...
export const getCalendarBookings = asyncHandler(async (req: Request, res: Response) => {
    ...
    const events = bookings.map(b => {
        ...
        return {
            ...
            status: b.status,
            destination: seg0?.country || seg0?.to || '',
        };
    });
