            ...
            if (booking.travelers && booking.travelers.length > 0) {
                reset({
                    travelers: booking.travelers.map(t => {
                        ...
                        // Read flight data from booking segments (source of truth), not passenger
                        ...
                        return {
                            ...
                            phoneNumber: pNumber,
                            email: t.email || '',
                            country: booking.destination || seg0?.country || booking.destinationCity || '',
                ...
                // We should initialize the primary traveler with booking-level contact and segment data.
                ...
                // Triple-layered fallback for dates
                const departureDate = seg0?.departureDate || booking.travelDate;
                const returnDate = seg0?.returnDepartureTime || seg0?.returnDate || booking.returnDate;
    ...
    const saveChangesMutation = useMutation({
        mutationFn: async (data: FormValues) => {
            ...
            if (tripTypeVal === 'multi-city') {
                // Leg 1 from primary traveler fields
                ...
                // Additional legs from segments state
                for (const seg of segments.filter(s => s.from || s.to)) {
                    enrichedSegments.push({
                        from: (seg.from || '').toUpperCase(),
                        to: (seg.to || '').toUpperCase(),
                        departureDate: seg.date ? new Date(seg.date).toISOString() : null,
                        returnDate: null,
                        returnDepartureTime: null,
                        tripType: 'multi-city',
                        country: null,
                ...
                // One-way or round-trip — single segment with all data
                enrichedSegments = [{
                    from: (primaryTraveler?.flightFrom || '').toUpperCase(),
                    to: (primaryTraveler?.flightTo || '').toUpperCase(),
                    departureDate: primaryTraveler?.departureTime ? new Date(primaryTraveler.departureTime).toISOString() : null,
            ...
            promises.push(api.put(`/bookings/${id}`, {
                ...
                additionalServicesDetails: additionalServicesDetails || null,
